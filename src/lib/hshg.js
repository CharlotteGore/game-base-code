...
function getAABB(obj){
    return {
        min : [obj.min.x, obj.min.y],
        max : [obj.max.x, obj.max.y],
        'static' : obj['static']
    }
}
...
function update_RECOMPUTE(){
    ...
    for(i = 0; i < this._globalObjects.length; i++){
        ...
        // recompute hash unless it's a static object... 
        if (!obj['static']){
            objAABB = getAABB(obj);
            ...
            for(k = 0; k < cell.objectContainer.length; k++){
                ...
                for(l = k+1; l < cell.objectContainer.length; l++){
                    ...
                    if(broadOverlapTest(objA, objB) === true){
                        possibleCollisions.push( [ objA.entity, objB.entity ] );
                    }
                }
            }
